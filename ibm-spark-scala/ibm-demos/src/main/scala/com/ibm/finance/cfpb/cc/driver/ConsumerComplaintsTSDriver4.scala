...
/**
  ...
  */
object ConsumerComplaintsTSDriver4 {
  ...
  def main(args: Array[String]) {
    ...
    @transient val sparkConf = new SparkConf()
      .setAppName("ConsumerComplaintsTSDriver4")
//      .setMaster("local[8]")
...
//    sc.stop()
  ...
  def generateTimeSeriesModel( sc: SparkContext): Long = {
    ...
    val consumerComplaintsRawDF = MongoSpark.load[ConsumerComplaints](sqlContext)
    ...
    // Save to DB
    val writeConfig = WriteConfig(Map("uri"-> MONGODB_URI), Some(WriteConfig(sc)))
